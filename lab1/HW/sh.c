...
runcmd(struct cmd *cmd)
{
  ...
  case ' ':
    ...
    // exec command
      ...
      const char* binPath1 = "/bin/";
      int pathLen1 = strlen(binPath1) + strlen(ecmd->argv[0]);
      char *accessPath1 = (char *)malloc((pathLen1+1)*sizeof(char));
      strcpy(accessPath1,binPath1);
      strcat(accessPath1,ecmd->argv[0]);

      const char* binPath2 = "/usr/bin/";
      int pathLen2 = strlen(binPath2) + strlen(ecmd->argv[0]);
      char *accessPath2 = (char *)malloc((pathLen2+1)*sizeof(char));
      strcpy(accessPath2,binPath2);
      strcat(accessPath2,ecmd->argv[0]);

      // printf("%s\n",accessPath);
      // try open file under bin fileHolder
      if(access(accessPath1,F_OK)==0){
        execv(accessPath1,ecmd->argv);
      }
      else if(access(accessPath2,F_OK)==0){
        execv(accessPath2,ecmd->argv);
      }
      else{
